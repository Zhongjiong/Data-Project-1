...
###delete empty data
data<- data[-which(apply(dummy, 1, sum)<3),]
dummy <- dummy[-which(apply(dummy, 1, sum)<3),]
...
cluster.data <- data.frame( business = apply (dummy[,business], 1, sum),
                              ...
                              bigdataskills = apply(dummy[,bigdata], 1, sum),
...
write.csv(data.re, 'data.result.csv')
...
write.csv(result, 'cluster.summary.csv')
